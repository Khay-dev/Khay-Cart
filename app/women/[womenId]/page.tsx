...
const WomenProductPage = ({ params }: any) => {
  const productId = params.womenId;
...
interface CartData {
  userId: number;
  date: string;
  products: { womenId: number; quantity: number }[];
}
...
const ProductDetailMainbar = ({ productData, isLoading }: ProductProps) => {
  ...
  const handleAddToCart = async () => {
    ...
    const cartData: CartData = {
      userId: 5,
      date: formattedDate,
      products: [{ womenId: productData.id, quantity: 1 }],
    };
    ...
    <div className="flex flex-col gap-y-5 basis-[35%] h-fit shrink-0 ">
      <div>
        <span className="font-inter font-normal text-xs text-gray">
          <Link href="/men">Women</Link> / {productData.category}
        </span>
